...
with equipment:
    ...
    with weapon:
        ...
        if not choice1 and not choice2:
            ...
            st.table(weapon_table[weapon_table['무기 종류'].isin(choice1)])
        elif not choice1 and choice2:
            st.table(weapon_table[weapon_table['속성'].isin(choice2)])
        else:
            st.table(weapon_table[weapon_table['무기 종류'].isin(choice1) & weapon_table['속성'].isin(choice2)])
with skill:
    st.subheader("장비 스킬")
